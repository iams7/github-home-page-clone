...
const calc = (x: number, y: number): [number, number, number] => [-(y - window.innerHeight / 2) / 360, (x - window.innerWidth / 2) / 240, 1.02];
...
const HoverCard: React.FC<Props> = ({ children, backgroundColor, direction, left }) => {
    ...
    const calcTranslate = (coordinate: number, containerSize: number, itemSize: number) => (coordinate / containerSize) * (containerSize - itemSize);

    const translateX = typeof window !== 'undefined' ? calcTranslate(cursorCoords.x - 450, window.innerWidth, 600) : 0;
    const translateY = typeof window !== 'undefined' ? calcTranslate(cursorCoords.y - 250, window.innerHeight, 500) : 0;
